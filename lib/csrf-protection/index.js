...
function csurf (options) {
  ...
  var ignoredPathRegs = getIgnoredPathRegs(opts)
  ...
  return function csrf (req, res, next) {
    ...
    if (!ignoreMethod[req.method] && !isIgnoredPath(req.path, ignoredPathRegs) && !tokens.verify(secret, value(req))) {
...
function getIgnoredPathRegs (opts) {
  var ignoredPath = []
  if (opts && opts.ignoredPath) {
    for (var i = 0; i < opts.ignoredPath.length; i++) {
      var path = opts.ignoredPath[i]
...
function isIgnoredPath (path, ignoredPathRegs) {
  if (ignoredPathRegs) {
    for (var i = 0; i < ignoredPathRegs.length; i++) {
      var reg = ignoredPathRegs[i];
      if (reg.exec(path) != null) {
        return true
      }
    }
  }
